...
const CustomStatusStyles = (status: string, info?: string): string => {
  switch (status) {
    case "COMPLETED": {
...
export const CalendarByDay: React.FC = () => {
          ...
          <Grid item xs={11}>
            <Grid container sx={{ display: "flex", gap: "10px" }}>
              {Array.from({ length: 7 }).map((_, colIndex) => {
                const doctor = currentDoctors[colIndex];

                const columnEvents = doctor ? doctor.events || [] : [];
                  ...
                  <Grid item xs key={colIndex} style={{ position: "relative" }}>
                    {hours.map((hour, rowIndex) => {
                      const eventsForCurrentHour = columnEvents.filter((event) => {
                        ...
                        return eventDate === currentDate && eventTime === hour;
                      });
